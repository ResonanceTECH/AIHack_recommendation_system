...
### ⚠️ Важно: Docker сборка временно недоступна

В текущей версии проекта есть проблемы с Docker сборкой frontend из-за конфликтов TypeScript конфигурации. Рекомендуется использовать локальную разработку.
...
CREATE USER 'medai_user'@'localhost' IDENTIFIED BY 'medai_password';
...
CREATE DATABASE medai_db;
CREATE USER medai_user WITH PASSWORD 'medai_password';
...
# Запустите базу данных и примените миграции
...
### Альтернативный запуск (Docker только для базы данных)

Если хотите использовать Docker только для базы данных:
